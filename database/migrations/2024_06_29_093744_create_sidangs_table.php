...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('sidangs', function (Blueprint $table) {
            ...
            $table->foreignId('pembimbing1_id')->constrained('dosens', 'id_dosen')->onDelete('cascade');
            $table->foreignId('pembimbing2_id')->constrained('dosens', 'id_dosen')->onDelete('cascade');
            $table->foreignId('ruangan_id')->constrained('ruangans', 'id_ruangan')->onDelete('cascade');
            $table->date('tanggal');
            $table->foreignId('jam_sidang_id')->constrained('ruangans', 'id_ruangan')->onDelete('cascade');
            $table->string('status_sidang', 20)->nullable();
            $table->decimal('total_nilai', 5, 2)->nullable();
            $table->timestamps();

            $table->foreign('nim')->references('nim')->on('tugas_akhirs')->onDelete('cascade');
        });
    }
